...
$query_vital_day = "SELECT * FROM vital_day_all order by vha_id_d desc limit 15";
    if ($result_vital_day = mysqli_query($mysqli, $query_vital_day))
    {
        $vha_time_d = '';
        $vha_real_price_d = '';
        $vha_sales_d = '';
        while( $row_vital_day = mysqli_fetch_assoc($result_vital_day) ){
            $vha_time_d .= $row_vital_day['vha_time_d'].",";
            $vha_real_price_d .= $row_vital_day['vha_real_price_d'].",";
            $vha_sales_d .= $row_vital_day['vha_sales_d'].",";
        }
        $vha_time_d = substr($vha_time_d, 0, -1);
        $vha_real_price_d = substr($vha_real_price_d, 0, -1);
        $vha_sales_d = substr($vha_sales_d, 0, -1);
    }
?>
<!doctype html>
<html lang="en">
...
<body>
							...
							<div class="panel">
								...
								<div class="panel-body no-padding">
									<table class="table table-striped">
										...
										<tbody>
                                            <?php 
                                            $query = "SELECT * FROM payment_list order by pay_id desc limit 5";
                                            if ($result = mysqli_query($mysqli, $query))
                                            {
                                            while( $row = mysqli_fetch_assoc($result) ){
                                                $item_id = $row['pay_shop'];
                                                $query_item = "SELECT tl_name FROM teacher_list where tl_id = '{$item_id}'";
                                            ...
                                            ?>
											<tr>
												<td><?php echo $row['pay_member'];?></td>
												<td><?php echo $row_item['tl_name'];?></td>
												...
												<td>
                                                    ...
                                                    if ($row['pay_status'] == 1) {
                                                        ...
                                                        echo "<span class=\"label label-warning\">未支付</span>";
                                                    } else if ($row['pay_status'] == 0) {
	...
	$(function() {
		...
		// headline charts
		data = {
			labels: [<?php echo $vha_time_d;?>],
			series: [
				[<?php echo $vha_real_price_d;?>],
				[<?php echo $vha_sales_d;?>],
			]
		};
		...
		// visits trend charts
		data = {
			labels: ['201801', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
			series: [{
				name: 'series-real',
