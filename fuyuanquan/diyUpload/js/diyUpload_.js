(function($){
	$.fn.extend({
		...
		if(opt.url){
			opt.server=opt.url;delete opt.url;
		}

		if(opt.success){
			var successCallBack=opt.success;delete opt.success;
		}
		if(opt.error){
		...
		webUploader.on('uploadProgress',function(file,percentage){
			...
			$diyBar.show();percentage=percentage*100;
	...
	function createBox($fileInput,file,webUploader){
		...
		$('#new_pic div p a.upload-btn').on('click',function(){
			// $("#circle").show();
			webUploader.upload();
			$('#new_pic div p a.upload-btn').text('暂停上传').one('click',function(){
					// $("#circle").hide();
					webUploader.stop();
		...
		if(file.type.split("/")[0]!='image'){
			var liClassName=getFileTypeClassName(
					file.name.split(".").pop()
				);
