...
if (strstr($agent_user,"MicroMessenger")){
	if (!$openids && !$code) {	
		header("Location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx95d36b67aa0c30e1&redirect_uri=http%3a%2f%2ffyq.shengtai114.com%2fscan_pay.php&response_type=code&scope=snsapi_userinfo&state=$id#wechat_redirect");
		exit;
	}
	if ($code) {
		...
		//$content = file_get_contents('https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx95d36b67aa0c30e1&secret=ddda7c309dc665b276238237598471e9&code='.$_REQUEST['code'].'&grant_type=authorization_code');
		...
		header("Location: http://fyq.shengtai114.com/scan_pay.php?openid=$openid&bid=$state");
		exit;
	}
}
...
<div class="scan_pay">
	...
	<div class="scan_pay_content">
		...
		<input type="number" step="0.01" name="pay_price" value="" placeholder="请输入付款金额">
		<input type="number" pattern="[0-9]*" name="pay_phone" value="<?php if ($mb_wxmember) {echo $mb_wxmember;}?>" placeholder="请输入手机号码">
		<p class="scan_pay_sale">打<font color="#f86342"><?php echo $row['tl_price']/10;?>折</font>后应付 : <font color="#f86342">0￥</font></p>
...
<script type="text/javascript">
	function scan_pay_button(){ 
		...
		if(!(/^1[34578]\d{9}$/.test(phone))){
			...
			$.post("post/scan_reg.php",
			  {
				recommend_phone:phone,
				recommend_vid:"<?php echo $row['tl_id'];?>",
				recommend_vphone:"<?php echo $row['tl_phone'];?>",
	...
	$(".scan_pay_content [name='pay_price']").keyup(function () {
		...
		$(this).val(txt);
		var scan_pay_sale = ($(this).val()*<?php echo $row['tl_price'];?>)/100;
		$(".scan_pay_sale font:eq(1)").text(scan_pay_sale.toFixed(2)+"￥");
	}).change(function () {
		$(this).keypress();
		var v = $(this).val();
		if (/\.$/.test(v))
