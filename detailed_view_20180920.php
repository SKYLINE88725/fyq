...
$picUrl = 'http://fyq.shengtai114.com/detailed_view.php?view='.$tid.'&type='.$type.'&mphone='.$member_login.'&qid='.$member_id;//二维码扫描出的链接
...
<div class="tc_detailed_">
	<ul>
		...
		<li class="tc_detailed_line2">
			<p class="tc_detailed_price">
				<span>
					...
					?>
						￥<?php echo $row['tl_price'];?><i style="margin-left: 5px; text-decoration: line-through; color: #959595; font-size: 0.8em;">￥<?php echo $row['tl_original'];?></i>
		...
		<li class="tc_detailed_line3">
			...
			?>
			<p class="tc_detailed_share_txt" onClick="wxshare('<?php echo utf8_strcut(wxstrFilter($row['tl_name']),35,'');?>', '<?php echo utf8_strcut(wxstrFilter($row['tl_summary']),40,'');?>', 'http://fyq.shengtai114.com/img/logo.png', '<?php echo $picUrl;?>')">点击分享</p>
			<?php 
				} else {
			?>
		...
		<ul>
			...
			<li>
				...
				<p class="detailed_view_region">
					<font color="#828282">
						<?php echo $row_view['cl_province'];?>
						<?php echo $row_view['cl_city'];?>
						<?php echo $row_view['cl_area'];?>
					</font>
				</p>
			</li>
			<li><a href="user_blog.php?id=<?php echo $row_view['cl_id'];?>" target="_self">进入店铺</a>
			</li>
		</ul>
		<ul>
			<li><a href="#" target="_self">全部宝贝 <font color="#707070"><?php echo $row_view['cl_allcount'];?></font></a>
			</li>
			<li><a href="my_commodity_add1.php" target="_self">全部销量 <font color="#707070"><?php echo $row_view['cl_allsales'];?></font></a>
			</li>
...
<div class="tc_detailed_foot" style="z-index: 100;">
	<ul>
		...
		if ($row['tl_point_type'] == "3") {
		?>
		<li class="item_scan"><a href="scan_pay.php?bid=<?php echo $row['tl_id'];?>" target="_self">立即支付</a></li>
		<?php 
		}
		?>
        <?php 
		if ($row['tl_point_type'] == "4") {
		?>
		<li class="item_scan"><a href="vip_card.php?bid=<?php echo $row['tl_id'];?>" target="_self">立即购买</a></li>
		<?php 
		}
		?>
	</ul>
...
if ($type == "company") {
?>
		<li class="tc_detailed_confirm_squantity"> <span>购买数量</span>
...
<div class="view_qrcode">
	<ul>
		<li><img src="<?php echo $row['tc_mainimg'];?>" alt="">
		</li>
		<li>
...
<script type="text/javascript">
		...
		var shipping_address = $.trim($('.tc_detailed_line4 a:nth-child(2) p').text());
		if ( mb_id ) {
			if (shipping_address) {
