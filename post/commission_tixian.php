...
$mb_not_gold = $row_member['mb_commission_not_gold'];
...
if ($result_receipt = mysqli_query($mysqli, $query_receipt))
{
	...
	$mb_commission_not_gold = $row_receipt['mb_commission_not_gold'];
	$before_money = $mb_commission_not_gold;
	$after_money = $mb_commission_not_gold - $tixian_commission;
}
...
if ($name_receipt && ($tixian_wechat || $tixian_bank || $tixian_alipay)) {
	if ($mb_not_gold<100) {
        ...
        if ($balance_zero > 5) {
            $sql_tixian = mysqli_query($mysqli,"INSERT INTO balance_details (t_money, t_way, t_status, t_phone, t_caption, t_description, t_cate, t_point, t_before_money, t_after_money, t_service_money, t_service_point) VALUES ('{$tixian_m}', '{$tixian_cate}', '0', '{$member_id}', 'commission_money', '佣金提现', 'commission_less', '{$com_point_sui}', '{$before_money}', '{$after_money}', '{$tixian_m_shui}', '{$com_point_sui}')");
            if ($sql_tixian) {
                $sql_member = mysqli_query($mysqli,"UPDATE fyq_member SET mb_commission_not_gold = mb_commission_not_gold-'{$tixian_commission}', mb_commission_finish_gold = mb_commission_finish_gold+'{$tixian_m}', mb_commission_finish_count = mb_commission_finish_count+1, mb_point = mb_point-$com_point_sui WHERE mb_phone = '{$member_id}'");
                if ($sql_member) {
                    echo 1;
                    $post_data = array(
                        'phone_num' => $member_id,
                        'phone_type' => 'expre_commission',
