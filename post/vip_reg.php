...
$query_limit = "SELECT item_limit FROM teacher_list where tl_id = '{$recommend_vid}'";
if ($result_limit = mysqli_query($mysqli, $query_limit))
{
    $row_limit = mysqli_fetch_assoc($result_limit);
    $item_limit = $row_limit['item_limit'];
    if ($item_limit>0) {
        $buy_sql = "SELECT buy_count FROM item_limit where item_id = '{$recommend_vid}' and user_id = '{$recommend_phone}'";
        if ($result_buy = mysqli_query($mysqli, $buy_sql))
        {
            $row_buy = mysqli_fetch_assoc($result_buy);
            if ($row_buy['buy_count']>=$item_limit) {
                echo 20;
                exit();
            }
        }
...
$sql_payment = mysqli_query($mysqli,"INSERT INTO payment_list (pay_member, pay_cate, pay_status, pay_shop, pay_count, pay_price, pay_trade_no, payment_types, mb_receiving_address) VALUES ('{$recommend_phone}', 'partner', '0', '{$recommend_vid}', '1', '{$recommend_price}', '{$trade_no}', '0' , '{$shipping_address}')");
