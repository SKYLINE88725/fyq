...
if ($other>0) {
	$query_code = "SELECT id,pay_code FROM payment_code where user_id = '{$user_id}' limit 1";
...
$query_member = "SELECT mb_id FROM fyq_member where mb_phone = '{$recommend_phone}' limit 1";
if ($result_member = mysqli_query($mysqli, $query_member))
{
    ...
    if ($member_rows) {
        ...
        $query_recommend = "SELECT * FROM fyq_member where mb_phone = '{$recommend_itemphone}' limit 1";
...
$sql_payment_oder = mysqli_query($mysqli,"UPDATE payment_list SET pay_member = '{$recommend_phone}',other_price = '{$other}' where pay_trade_no = '{$recommend_trade}'");
if ($sql_payment_oder) {
