...
$query = "SELECT * FROM subject_list where sub_id = '{$sub_id}'";
...
<style type="text/css">
		...
		<ul>
        	<li>
                <div class="input-group" style="float: left; margin-left: 10px; width: 90%; margin-top:10px;">
                    <label class="shanggpin_label input-group-addon">科目标题</label>
                    <input type="text" class="form-control" placeholder="请输入科目标题" name="sub_title" value="<?php echo $row['sub_title'];?>" >
                </div>            
                <input class="form-control" type="hidden" alt="商家店铺号" name="sub_teacher_id" value="<?php echo $row['sub_teacher_id'];?>">
                <input class="form-control" type="hidden" alt="商家手机号" name="sub_phone" value="<?php echo $member_login;?>">
			</li>
            <li>
                <div class="input-group" style="float: left; margin-left: 10px; width: 90%; margin-top:10px;">
                    <div style="font-weight: bold; margin-bottom: 5px;">科目内容</div>
                    <textarea class="form-control" placeholder="请输入科目内容" rows="4" name="sub_desc" ><?php echo $row['sub_desc'];?></textarea>
                </div>            

			</li>
			<li>
				<div style="float: left; margin-left:10px; width: 90%; margin-top:10px;">
					<label id="spic" class="mui-btn mui-btn-primary">选择科目图片</label>
					<input type="file" name="file1" class="inputfile" style="display:none;">
                    <div id="feedback" style="/*float: left;*/ width: 50%;margin: 15px auto;"><img src="<?php echo $row['sub_picture'];?>" alt="" style="width:100%;"></div>
	...
	<script type="text/javascript">
		$( "#subject_in" ).click( function () {
            ...
            if ( !sub_title || !sub_cate_cid || !sub_desc) {
                var msgstr = "内容填写不完整!";
                if(!sub_title)
                {
                    alert('请填写活动标题！');  
			...
			else 
			{
                ...
                $.post( "post/subject_alter_post.php", {
